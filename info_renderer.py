from util import friendly_date_string, api_url, remove_protocol
...
class InfoRenderer:
    ...
    def render_info(self, total_tasks, tasks_rendered):
        ...
        info_text = f"{self.pisugar.get_battery_percent()}% | {remove_protocol(api_url())} | {friendly_date_string(datetime.now())} | {tasks_rendered}/{total_tasks} tasks"
